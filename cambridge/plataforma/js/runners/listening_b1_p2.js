...
export function initListeningB1Part2(exercises, skill, part) {
  exercise = null;
  answers = {};
  _skill = skill;
  _part = part;
...
function renderExercise(exercises) {
  ...
  exercise.questions.forEach((q, i) => {
    ...
    // Para la parte 2, mostramos el Contexto en cursiva y luego la Pregunta
    div.innerHTML = `
      <div class="card-title">Question ${i + 8}</div>
    ...
    html += `
      <div class="card" style="border-left: 4px solid ${statusColor}">
        <div class="card-title" style="color: ${statusColor}; display: flex; align-items: center; gap: 0.5rem;">
          Question ${i + 8} <span>${isCorrect ? '✓' : '✗'}</span>
        </div>
        ${q.context ? `<p style="font-style: italic; color: var(--color-text-muted); margin-bottom: 0.5rem;">${q.context}</p>` : ''}
        <p style="font-weight: 600; margin-bottom:1rem">${q.question}</p>
        ...
        <div style="display:flex; flex-direction:column; gap:0.5rem;">
          ${q.options.map((opt, j) => {
            ...
            if (j === cAns && j === uAns) {
              ...
              icon = '<span style="color:#22c55e; font-weight:bold; margin-left:auto">✓ Correct</span>';
            } else if (j === cAns) {
              borderColor = '#22c55e'; bgColor = '#f0fdf4';
              icon = '<span style="color:#22c55e; font-weight:bold; margin-left:auto">✓ Answer</span>';
            } else if (j === uAns) {
