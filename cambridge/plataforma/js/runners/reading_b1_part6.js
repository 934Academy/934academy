...
export function initReadingOpenCloze(exercises, skill, part) {
  exercise = null;
  answers  = {};
  _skill   = skill;
  _part    = part;
...
function renderSelector(exercises) {
  ...
  c.innerHTML = `
    <div class="page-header">
      <h1 class="page-title">Part 6 — Open Cloze Text</h1>
...
function buildGappedText() {
  ...
  exercise.questions.forEach((q, i) => {
    // Usamos el número propio de la pregunta si existe (ej. 28, 29...), o calculamos el fallback
    const num = q.number || (i + 27);
    
    // Posibles formatos de marcadores en el texto del ejercicio
    const markerLong  = `(${num}) ____`;
    const markerShort = `(${num}) ___`;
    ...
    const input = `<input type="text" 
      ...
      autocomplete="off" 
      spellcheck="false"
      style="width:110px; padding:4px 8px; border-radius:6px; border:2px solid var(--color-border);
      background:var(--color-surface); color:var(--color-text); text-align:center;
      font-weight:600; text-transform:uppercase;" 
      value="${answers[q.id] || ''}"/>`;

    // Intentar reemplazar por marcador específico con número, si no, consume el siguiente grupo de guiones bajos disponible
    if (text.includes(markerLong)) {
      text = text.replace(markerLong, input);
    } else if (text.includes(markerShort)) {
      text = text.replace(markerShort, input);
    } else {
      text = text.replace(/____+|___+/, input);
    }
  });
...
function renderExercise(exercises) {
  ...
  // Control estricto de entrada por teclado por cada input
  c.querySelectorAll('input[data-id]').forEach(inp => {
    inp.addEventListener('input', () => {
      // Elimina espacios en blanco globales y fuerza mayúsculas en tiempo real en la pantalla
      inp.value = inp.value.replace(/\s+/g, '').toUpperCase();
      // Guarda la palabra limpia en el estado
